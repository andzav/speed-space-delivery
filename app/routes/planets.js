        ...
        userModel.findOne({
            ...
            if (err) res.status(400).send('Error while querying database');
                ...
                if (person.permission === 'admin' || person.permission === 'operator') {
                    ...
                    planetModel.find(query, '-_id -__v', {
                        sort: {
                            id: 1
                        }
                    }, function (err, result) {
                    ...
                    planetModel.find({}, 'name moonOf galactic -_id', {
                        sort: {
                            id: 1
                        }
                    }, function (err, result) {
        ...
        userModel.findOne({
            ...
            if (err) res.status(400).send('Error while querying database');
                ...
                if (person.permission === 'admin') {
                    if (newPlanet !== undefined && newPlanet.name !== undefined && newPlanet.type !== undefined && newPlanet.galactic !== undefined &&
    ...
    .put(function (req, res) {
        ...
        userModel.findOne({
            ...
            if (err) res.status(400).send('Error while querying database');
                ...
                if (person.permission === 'admin') {
                    if (newPlanet !== undefined && newPlanet.name !== undefined) {
                        planetModel.findOne({
                            name: newPlanet.name
                        }, function (err, planet) {
                            if (err) res.status(400).send('Error while querying planet database');
