                    ...
                    if (person.modification === 'unconfirmed') {
                        ...
                        if (person.modification !== "changingPass") {
                            ...
                            if (person.sessions.length > 0 && findIndex !== -1) {
                                let response = {
                                    'SID': person.sessions[findIndex].SID,
                        ...
                        if (err) res.status(400).send('Error while querying database');
                            ...
                            userModel.findOne({
                                ...
                                if (err) res.status(400).send('Error while querying database');
                                    ...
                                    let operator = new userModel({
                                        'email': email,
                                        'password': pass,
                                        'location': location,
                                        'salt': salt,
                                        'modification': 'unconfirmed',
                                        'permission': 'operator'
                                    });
        ...
        if (!validator.validate(email) || !password || password.length < 8) {
            ...
            userModel.findOne({
                ...
                if (err) res.status(400).send('Error while querying database');
                    ...
                    let salt = crypto.createHash('sha256').update('SSD' + email + 'LUL').digest('hex');
                    password = crypto.createHash('sha256').update(password + salt).digest('hex');
                    let cCode = randomstring.generate({
                        length: 8,
                        charset: 'numeric'
                    });
                    let unconfirmedUser = new unconfirmedModel({
                        'email': email,
                        'password': password,
                        'salt': salt,
                        'cCode': cCode
...
router.route('/confirm')
    .post(function (req, res) {
        ...
        if (!validator.validate(email) || cCode === undefined) {
            ...
            unconfirmedModel.findOneAndRemove({
                'email': email,
                'cCode': cCode
            }, '-_id email password salt', function (err, result) {
                if (err) res.status(400).send('Error while querying database');
                else if (result) {
