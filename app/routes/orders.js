...
router.route('/')
    .get(function (req, res) {
        ...
        }, 'permission email location', function (err, person) {
                    ...
                    orderModel.findOne(query, function (err, order) {
                        if (err) res.status(400).send('Error while querying orders database');
                            ...
                            if (action === 'return') {
                                ...
                                let returnOrder = new orderModel({
                                    'reciever': order.sender,
                                    'sender': order.reciever,
                                    'from': order.to,
                                    'to': order.from,
                                    'weight': order.weight,
                                    'volume': order.volume,
...
router.route('/createContainer')
    .post(function (req, res) {
        ...
        userModel.findOne({
            ...
            'sessions.ip': ip,
            'sessions.fingerprint': req.fingerprint.hash
        }, 'permission location', function (err, person) {
            if (err) res.status(400).send('Error while querying database');
            else if (person) {
                if (person.permission === 'operator' || person.permission === 'admin') {
                        ...
                        if (query.location) {
                            orderModel.find(query, function (err, result) {
                                if (err) res.status(400).send('Error while querying orders database');
                                    ...
                                    let container = new containerModel({
                                        'ordersIDArray': orderArray,
                                                                ...
                                                                if (typeof calculations !== 'string') {
                                                                    ...
                                                                    container.save(function (err, scontainer) {
                                                                        if (err) res.status(400).send('Error while saving container to database');
                                                                            ...
                                                                            let response = {
                                                                                'id': scontainer.id,
                                                                                'options': calculations.properties
                                                                            };
                                                                            res.json(response);
                                                                        }
                                                                    });
            ...
            if (err) res.status(400).send('Error while querying database');
                ...
                if (person.permission === 'operator' || person.permission === 'admin') {
                    if (containerID) {
                        ...
                        }; //can't accept container without a ship
        ...
        userModel.findOne({
            ...
            if (err) res.status(400).send('Error while querying database');
                ...
                if (person.permission === 'operator') {
                    let query = {
                        $or: [{
                            source: person.location
                        }, {
                            destinationsArray: person.location
                        }],
                        'destinationsArray': {
                            $exists: true,
                            $ne: []
                    ...
                    if (id) query.id = id;
                    containerModel.find(query, '-_id -__v', {
                        sort: {
                            id: 1
                        }
...
router.route('/track/:trackID(\\d+)')
    .get(function (req, res) {
        ...
        }, '-_id -__v -containerID -sender -reciever', function (err, order) {
