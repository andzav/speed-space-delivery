...
router.route('/')
    .get(function (req, res) {
        ...
        }, 'permission email', function (err, person) {
        ...
        userModel.findOne({
            ...
            if (err) res.status(400).send('Error while querying database');
                ...
                if (person.permission === 'admin') {
                    if (newPath !== undefined && newPath.source !== undefined && newPath.target !== undefined && newPath['length'] !== undefined &&
        ...
        userModel.findOne({
            ...
            if (err) res.status(400).send('Error while querying database');
                ...
                if (person.permission === 'admin') {
                    ...
                    if (query.id !== undefined || query.source !== undefined) {
        ...
        userModel.findOne({
            ...
            if (err) res.status(400).send('Error while querying database');
                ...
                if (person.permission === 'admin') {
                    ...
                    } else res.status(400).send('No parameters given for path');
                } else res.status(401).send('Not enough permission');
            } else res.status(401).send('User not found');
        });
    });
